<?php
	...
	/* MODELO DE CARROUSEL */
	$Conteudo = '<div id="chamadas-principais" class="carousel slide">';
       ...
       $Conteudo .= '<div class="carousel-inner">';
			...
			if ( $query->have_posts()){
				...
				while ( $query->have_posts() ) {
					...
					$Imagens = '<a href="'.get_field('link_post_area1').'"><img class="img-responsive" src="'.$objImagem["url"].'"  alt="'.$objImagem["alt"].'"  title="'.$objImagem["title"].'"  /></a>';
					
					if ($i == 0){
						$Conteudo .= '<div class="item active">';
					}else{
						$Conteudo .= '<div classe="item">';
					}
							$Conteudo .= $Imagens;
